  ...
  checkCameraAuth() {
    ...
    wx.getSetting({
      success(res) {
        ...
        // 不再在页面加载时调用 wx.authorize，改为在拍照按钮点击时引导
      },
      fail() {
    ...
    wx.compressImage({
      ...
      success: (res) => {
        ...
        if (plantId) {
          ...
          // 识别模式：跳转到识别结果页
          wx.navigateTo({
            url: `/package-camera/pages/result_swiper/result_swiper?tmp_filePath=${encodeURIComponent(res.tempFilePath)}`
          });
        }
      },
      fail: () => {
        ...
        if (plantId) {
          ...
          wx.navigateTo({
            url: `/package-camera/pages/result_swiper/result_swiper?tmp_filePath=${encodeURIComponent(imagePath)}`
          });
        }
      }
    });
