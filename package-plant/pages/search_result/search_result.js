...
Page({
  ...
  onLoad(options) {
    ...
    if (identifyDataStr) {
      try {
        identifyData = JSON.parse(identifyDataStr);
      } catch (e) {
        console.error('[search_result] identify_data JSON 解析失败:', e);
        identifyData = null;
      }
    }
    ...
    if (searchText) {
      this.searchPlant(searchText, scientificName, hasImage, hasIdentifyData, identifyData, imageUrl);
    } else {
   ...
   * @param {string} imageUrl - 传递的图片 URL（已知植物）
   */
  async searchPlant(keyword, providedScientificName, hasImage, hasIdentifyData, identifyData, imageUrl) {
    ...
    if (hasImage) {
      ...
      // 关键修复：使用传递的 imageUrl，而不是预设的 imageUrl
      this.setData({ 
        loading: false, 
        error: false, 
        plant: {
          ...presetPlant,
          imageUrl: imageUrl  // 使用发现页传递的图片
        },
        headerImage: imageUrl,  // 确保头图使用传递的图片
        difficultyStars: stars
      });
      this.checkFavorite();
      return;
    ...
    try {
      ...
      const needImageSearch = !aiResult || !aiResult.success || !aiResult.imageUrl;
      let searchedImageUrl = '';
      
      if (needImageSearch) {
        // 异步搜索图片，不阻塞页面显示
        searchedImageUrl = await this.searchPlantImageOnly(keyword);
      }
      
      if (!aiResult || !aiResult.success) {
        // 检查是否是"非植物名称"的提示
        if (aiResult && aiResult.isPlant === false) {
          this.setData({
            loading: false,
            error: true,
            errorMessage: aiResult.message || '请输入有效的植物名称'
          });
          return;
        }
        ...
        // 如果有搜索到图片，替换 fallback 的默认图片
        if (searchedImageUrl) {
          fallbackPlant.imageUrl = searchedImageUrl;
        }
        ...
        this.setData({ 
          ...
          plant: fallbackPlant, 
          difficultyStars: stars,
          headerImage: searchedImageUrl || fallbackPlant.imageUrl || ''
        });
        this.checkFavorite();
        return;
      }
      
      // 云函数成功：使用返回的数据，如果图片为空则用搜索到的图片
      const scientificNameToUse = providedScientificName || aiResult.scientificName || '';
      const imageUrlToUse = imageUrl || aiResult.imageUrl || searchedImageUrl || '';
      ...
      // 异常时也要尝试搜索图片
      const searchedImageUrl = await this.searchPlantImageOnly(keyword);
      const fallbackPlant = this.getFallbackPlant(keyword);
      const stars = this.generateDifficultyStars(fallbackPlant.difficultyLevel);
  ...
  /**
   * 单独搜索植物图片（当主云函数未返回图片时使用）
   * 使用 mode: 'image_only' 轻量模式，不调 AI，只搜图
   */
  async searchPlantImageOnly(keyword) {
    return new Promise((resolve) => {
      wx.cloud.callFunction({
        name: 'getCareGuide',
        data: {
        ...
        fail: (err) => {
          console.error('[search_result] 图片搜索失败:', err);
          resolve('');
        }
      });
    });
  ...
  onShareAppMessage() {
    return {
      ...
      path: `/package-plant/pages/search_result/search_result?search_text=${encodeURIComponent(this.data.searchText)}`
    };
  },
  ...
  previewImage() {
    ...
    let imageUrl = headerImage || plant?.imageUrl;

    if (!imageUrl) return;

    // 云存储 fileID 需要先获取临时 URL
