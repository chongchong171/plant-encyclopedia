...
exports.main = async (event, context) => {
  ...
  console.log('[getCareGuide] - PEXELS_API_KEY:', process.env.PEXELS_API_KEY ? '已配置（长度:' + process.env.PEXELS_API_KEY.length + '）' : '❌ 未配置');
  ...
  const imagePromise = (async () => {
    try {
      ...
      console.log('[getCareGuide] 图片搜索关键词:', standardPlantName, '→', latinNameToUse);
      
      const imageUrl = await getPlantImageFromFreeSources(
        latinNameToUse,  // 使用拉丁学名
        standardPlantName  // 使用中文名
      );
...
/**
 * 从免费公开源获取植物图片（使用 Pexels，智能关键词优化）
 */
async function getPlantImageFromFreeSources(latinName, chineseName) {
  const startTime = Date.now();
  console.log('[PlantImage] 开始获取图片:', latinName || '无拉丁名', '/', chineseName || '无中文名');
  console.log('[PlantImage] PEXELS_API_KEY 配置检查:', process.env.PEXELS_API_KEY ? '已配置（长度:' + process.env.PEXELS_API_KEY.length + '）' : '未配置');
  
  // 策略：优先使用拉丁学名/英文名，如果没有则使用中文名
  let searchQuery = latinName || chineseName;
  
  // 智能优化：只对纯中文名（不包含英文字母）且没有植物指示词的添加"植物"后缀
  if (!latinName && chineseName) {
    const hasEnglish = /[a-zA-Z]/.test(chineseName);  // 检查是否包含英文
    const hasPlantIndicator = /[花草树木竹藤藻菌]/.test(chineseName);
    
    if (!hasEnglish && !hasPlantIndicator) {
      searchQuery = chineseName + ' 植物';
      console.log('[PlantImage] 智能优化关键词:', chineseName, '→', searchQuery);
    } else {
      console.log('[PlantImage] 使用原始关键词:', chineseName);
    }
  }
  
  if (!searchQuery) {
    console.log('[PlantImage] ❌ 没有可用的搜索名称');
    return '';
  }
  
  console.log('[PlantImage] 最终搜索关键词:', searchQuery);
  
  // 尝试 Pexels（2.5 秒超时）
  try {
    console.log(`[PlantImage] ${Date.now() - startTime}ms - 开始 Pexels 查询...`);
    const queryUrl = `https://api.pexels.com/v1/search?query=${encodeURIComponent(searchQuery)}&per_page=1`;
    console.log('[PlantImage] 请求 URL:', queryUrl);
    console.log('[PlantImage] 使用 API Key:', process.env.PEXELS_API_KEY ? '前 10 位：' + process.env.PEXELS_API_KEY.substring(0, 10) + '...' : '未配置');
    ...
    console.log('[PlantImage] Pexels 响应状态:', searchRes.status);
    
    if (searchRes.ok) {
      const searchData = await searchRes.json();
      console.log('[PlantImage] 完整返回数据:', JSON.stringify(searchData).substring(0, 500));
      
      const imageUrl = searchData.photos?.[0]?.src?.large;
      console.log('[PlantImage] 提取的图片 URL:', imageUrl);
      
      if (imageUrl) {
        console.log(`[PlantImage] ${Date.now() - startTime}ms - ✅ Pexels 找到图片`);
        console.log('[PlantImage] 图片 URL:', imageUrl);
        return imageUrl;
      } else {
        console.log('[PlantImage] ❌ 返回数据中没有图片 URL');
      }
    } else {
      const errorText = await searchRes.text();
      console.error('[PlantImage] Pexels API 错误:', searchRes.status, errorText);
    }
    
    console.log(`[PlantImage] ${Date.now() - startTime}ms - ❌ Pexels 未找到图片`);
  } catch (err) {
    console.log(`[PlantImage] ${Date.now() - startTime}ms - ❌ Pexels 查询失败:`, err.message);
  }
  
  // 返回失败
  console.log(`[PlantImage] ${Date.now() - startTime}ms - ❌ 图片搜索失败`);
  return '';
}
