...
exports.main = async (event, context) => {
  ...
  const imagePromise = (async () => {
    try {
      ...
      // 2. 维基百科失败，降级 Pexels
      if (!imageUrl) {
        imageUrl = await getPlantImageFromPexels(standardPlantName);
      }
...
async function getPlantImageFromWiki(chineseName) {
  const startTime = Date.now();
  console.log('[PlantImage-Wiki] 开始获取图片:', chineseName || '无中文名');
  ...
  try {
    ...
    const queryUrl = `${WIKIPEDIA_SUMMARY_URL}${encodeURIComponent(chineseName)}`;
    ...
    if (searchRes.ok) {
      ...
      console.log('[PlantImage-Wiki] 完整返回数据:', JSON.stringify(searchData).substring(0, 500));
      
      // 提取图片 URL（优先原图，其次缩略图）
      const imageUrl = searchData.originalimage?.source || searchData.thumbnail?.source || '';
      console.log('[PlantImage-Wiki] 提取的图片 URL:', imageUrl);
      
      if (imageUrl) {
        console.log(`[PlantImage-Wiki] ${Date.now() - startTime}ms - ✅ 维基百科找到图片`);
        console.log('[PlantImage-Wiki] 图片 URL:', imageUrl);
        return imageUrl;
      } else {
        console.log('[PlantImage-Wiki] ❌ 返回数据中没有图片 URL');
      }
    } else {
      const errorText = await searchRes.text();
      console.error('[PlantImage-Wiki] API 错误:', searchRes.status, errorText);
    }
 ...
 */
async function getPlantImageFromPexels(chineseName) {
  const startTime = Date.now();
  console.log('[PlantImage-Pexels] 开始获取图片（降级方案）:', chineseName || '无中文名');
  ...
  // 尝试 Pexels（2.5 秒超时）
  try {
    console.log(`[PlantImage-Pexels] ${Date.now() - startTime}ms - 开始 Pexels 查询...`);
    const queryUrl = `https://api.pexels.com/v1/search?query=${encodeURIComponent(chineseName + ' 植物')}&per_page=1`;
    ...
    if (searchRes.ok) {
      ...
      if (imageUrl) {
        console.log(`[PlantImage-Pexels] ${Date.now() - startTime}ms - ✅ Pexels 找到图片`);
        return imageUrl;
      }
    }
    
    console.log(`[PlantImage-Pexels] ${Date.now() - startTime}ms - ❌ Pexels 未找到图片`);
  } catch (err) {
    console.log(`[PlantImage-Pexels] ${Date.now() - startTime}ms - ❌ Pexels 查询失败:`, err.message);
  }
